...
% NOISE
for i = 1:n_imm
    P_in{i} = Pgt{i} + .01*rand(size(Pgt{i}));
end
M_in = M  + .1*randn(size(M));
...
fprintf('Before Bundle Adjustment reproj error:\t %0.5g \n', err/n_imm );

[P_out,M_out] = bundleadj(P_in,M_in,K,m,vis);

err=0;
for i = 1:n_imm
    m_est = htx(P_out{i},M_out);  % project with estimated camera
    err= err+ norm( m{i} -m_est);
end
